...
c
      allocate(strdrop(3,nps),stat=ierr)
      if(ierr.ne.0)stop ' Error in sdmsdrop: strdrop not allocated!'
...
c
      do is=1,ns
        ...
        do ips=nps1(is),nps2(is)
          wei(ips)=0.d0
          sm(ips)=parea(ips)*dsqrt(slpmdl(1,ips)**2+slpmdl(2,ips)**2)
          smsum=smsum+sm(ips)
        enddo
c
        sm90=0.d0
...
c
        ...
        maxsdrop(is)=0.d0
        do ips=nps1(is),nps2(is)
          sdss=sdss+wei(ips)*strdrop(1,ips)
          sdds=sdds+wei(ips)*strdrop(2,ips)
          sdnn=sdnn+wei(ips)*strdrop(3,ips)
          maxsdrop(is)=dmax1(maxsdrop(is),
     &       strdrop(1,ips)**2+strdrop(2,ips)**2+strdrop(3,ips)**2)
        enddo
