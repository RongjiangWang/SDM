...
c     work space
c
      integer i,j,l,ir,nr,izs,ierr
      integer istp,isp,nr1,nr2,nzs,nls,nlr
      integer lend,lenf,leninp,iunit
      integer unit(3,4)
      double precision r1,r2,dr,rratio,dract
      double precision pi,accuracy,zdis
      double precision zs1,zs2,dzs,zratio,dzact,vp,vs
      double precision swap
      double precision zs0(nzsmax)
...
c
      ...
      if(r1.lt.0.d0.or.r2.lt.r1.or.nr.le.1)then
        ...
        stop 'Error: bad distance samples!'
...
c
      do i=1,l
        call getdata(10,dataline)
        read(dataline,*)j,h(i),vp,vs,rho(i)
        h(i)=h(i)*km2m
        vp=vp*km2m
        vs=vs*km2m
        mu(i)=rho(i)*vs*vs
...
c
      homog=.true.
      do i=2,l
        if(la(i).ne.la(i-1).or.
     &     mu(i).ne.mu(i-1).or.
     &     rho(i).ne.rho(i-1))homog=.false.
      enddo
      if(homog)then
...
800   continue
      if(nwarn.eq.0)then
        print *,'####################################################'
        print *,'#                                                  #'
        print *,'#          End of computations with DGRN08         #'
        print *,'#                                                  #'
        print *,'####################################################'
      else
        print *,'####################################################'
